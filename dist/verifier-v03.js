...
function canonicalize(obj) {
    return JSON.stringify(obj, (_, v) => {
        if (v && typeof v === "object" && !Array.isArray(v)) {
            return Object.keys(v).sort().reduce((s, k) => {
                s[k] = v[k];
                return s;
            }, {});
        }
...
export function verifyRecordV03(record, timestampToleranceMs = 1000, estimateTolerance = 1e-4) {
    const violations = [];
    const V = (v, cat, msg, id, chain) => violations.push({ version: v, category: cat, message: msg, checkId: id, chain });
    if (!record.studentScopeId)
        V(0, "schema", "Missing studentScopeId", "SCHEMA.REQUIRED", "record");
    const evidence = record.evidence || {};
    const evEntries = evidence.entries || [];
    ...
    for (const entry of evEntries) {
        ...
        for (const req of ["version", "timestamp", "eventHash", "event", "prevHash", "entryHash"]) {
            if (!(req in entry))
                V(v, "schema", `${req} is required`, "SCHEMA.REQUIRED", ch);
        }
        const ts = entry.timestamp || "";
        const tsMs = ts ? new Date(ts).getTime() : NaN;
        if (ts && isNaN(tsMs))
            V(v, "timestamp", `Not valid ISO 8601: ${ts}`, "SCHEMA.TIMESTAMP_FORMAT", ch);
        if (typeof v === "number" && v <= prevV)
            V(v, "version", `Version ${v} <= previous ${prevV}`, "VERSION.MONOTONIC", ch);
        if (prevTs && ts && !isNaN(tsMs)) {
            const pm = new Date(prevTs).getTime();
            if (!isNaN(pm) && tsMs < pm - timestampToleranceMs)
                V(v, "timestamp", "Timestamp goes backward", "TIMESTAMP.MONOTONIC", ch);
        }
        ...
        if (kind === "unknown")
            V(v, "schema", "Event is not Observation, Consent, or Redacted", "SCHEMA.EVENT_UNION", ch);
        if (kind === "observation") {
            if (event.scaffoldType && !VALID_ST.has(event.scaffoldType))
                V(v, "enum", `Invalid scaffoldType`, "ENUM.VALID", ch);
            if (event.epistemicMode && !VALID_EM.has(event.epistemicMode))
                V(v, "enum", `Invalid epistemicMode`, "ENUM.VALID", ch);
        }
        if (kind === "consent") {
            if (!VALID_CA.has(event.action))
                V(v, "enum", `Invalid consent action`, "ENUM.VALID", ch);
            for (const s of event.scopes || []) {
                if (!VALID_CS.has(s))
                    V(v, "enum", `Invalid consent scope: ${s}`, "ENUM.VALID", ch);
                consentState[s] = event.action;
            }
        }
        if (kind !== "redacted") {
            const computedEvHash = sha256(canonicalize(event));
            if (entry.eventHash !== computedEvHash)
                V(v, "hash", "eventHash mismatch", "EVENT.COMMITMENT", ch);
        }
        if (entry.prevHash !== prevH)
            V(v, "hash", "prevHash mismatch", "HASH.PREV_LINK", ch);
        ...
        if (entry.entryHash !== computedEntryHash)
            V(v, "hash", "entryHash mismatch", "HASH.ENTRY_RECOMPUTE", ch);
        evByVersion[v] = entry;
    ...
    for (let idx = 0; idx < estimates.length; idx++) {
        ...
        if (!estimator.name)
            V(0, "schema", "Estimator name missing", "ESTIMATE.DECLARED", chainLabel);
        ...
        const estEntries = estChain.entries || [];
        ...
        for (const entry of estEntries) {
            const v = entry.version ?? -1;
            if (typeof v === "number" && v <= estPrevV)
                V(v, "version", `Version ${v} <= previous ${estPrevV}`, "VERSION.MONOTONIC", chainLabel);
            if (entry.prevHash !== estPrevH)
                V(v, "hash", "prevHash mismatch", "HASH.PREV_LINK", chainLabel);
            const hashable = { ...entry };
            delete hashable.entryHash;
            const computed = sha256(canonicalize(hashable));
            if (entry.entryHash !== computed)
                V(v, "hash", "entryHash mismatch", "HASH.ENTRY_RECOMPUTE", chainLabel);
            const evVer = entry.evidenceVersion;
            if (evVer != null && !(evVer in evByVersion))
                V(v, "schema", `evidenceVersion ${evVer} not in evidence`, "ESTIMATE.LINKS_EVIDENCE", chainLabel);
            if (evVer != null && evVer <= lastEvV)
                V(v, "version", `evidenceVersion ${evVer} <= previous ${lastEvV}`, "ESTIMATE.ORDER", chainLabel);
            if (entry.triageResult && !VALID_TR.has(entry.triageResult))
                V(v, "enum", `Invalid triageResult`, "ENUM.VALID", chainLabel);
            if (entry.depthLevel && !VALID_DL.has(entry.depthLevel))
                V(v, "enum", `Invalid depthLevel`, "ENUM.VALID", chainLabel);
            if (entry.decisionTier && !VALID_DT.has(entry.decisionTier))
                V(v, "enum", `Invalid decisionTier`, "ENUM.VALID", chainLabel);
            const w = entry.evidentialWeight;
            if (typeof w === "number" && (w < 0 || w > 1))
                V(v, "schema", `Weight ${w} outside [0,1]`, "WEIGHT.RANGE", chainLabel);
            for (const f of ["priorPosterior", "updatedPosterior"]) {
                const pv = entry[f];
                if (typeof pv === "number" && (pv <= 0 || pv >= 1))
                    V(v, "schema", `${f} ${pv} outside (0,1)`, "POSTERIOR.RANGE", chainLabel);
            }
    ...
    for (const att of attestations) {
        if (att.payload && att.payloadHash) {
            ...
            if (computed !== att.payloadHash)
                V(0, "hash", "Attestation payloadHash mismatch", "ATTEST.INTEGRITY", "record");
        }
    ...
    return {
        ...
        level,
        violations,
        entriesChecked: evEntries.length + estimates.reduce((s, e) => s + (e.entries?.length || 0), 0),
        estimators: estimatorSummaries,
    };
}
//# sourceMappingURL=verifier-v03.js.map