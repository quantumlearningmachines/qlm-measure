...
function cmdVerifyChain(argv: string[]): number {
  const opts: Record<string, string> = {};
  const positional: string[] = [];
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === "--family" || a === "--scheme" || a === "--format") { opts[a.slice(2)] = argv[++i]; }
    else if (a === "--allow-mixed") { opts.allowMixed = "1"; }
    else if (a === "--list") { opts.list = "1"; }
    else positional.push(a);
  }
  if (opts.list) {
    for (const s of listSchemes()) console.log(`${s.id}\t${s.family}\tsince ${s.since}\t${s.hashField}/${s.prevField}`);
    return 0;
  }
  const file = positional[0];
  if (!file) { console.error("Usage: qlm-measure verify-chain <events.json> (--family F | --scheme S) [--allow-mixed] [--format text|json] | --list"); return 2; }
  if (!opts.family && !opts.scheme) { console.error("ERROR: pass --family (e.g. tpc/clinical) or --scheme (e.g. tpc/clinical-v2)"); return 2; }
  if (!existsSync(file)) { console.error(`ERROR: ${file} not found`); return 2; }
  let events: unknown;
  try { events = JSON.parse(readFileSync(file, "utf-8")); } catch (e) { console.error(`ERROR: parse failed: ${e}`); return 2; }
  if (!Array.isArray(events) && events && typeof events === "object" && Array.isArray((events as { events?: unknown }).events)) {
    events = (events as { events: unknown[] }).events; // { events: [...] } wrapper (vectors, exports with metadata)
  }
  if (!Array.isArray(events)) { console.error("ERROR: not an array of events (or an object with an events array)"); return 2; }
  let r;
  try { r = verifyChain(events, { family: opts.family, scheme: opts.scheme, rejectMixed: !opts.allowMixed }); }
  catch (e) { console.error(`ERROR: ${(e as Error).message}`); return 2; }
  if (opts.format === "json") { console.log(JSON.stringify({ verifier: `qlm-measure ${VERSION}`, file, ...r }, null, 2)); return r.clean ? 0 : 1; }
...
switch (cmd) {
  ...
  case "verify-chain": exitCode = cmdVerifyChain(args.slice(1)); break;
